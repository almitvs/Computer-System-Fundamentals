...
struct TestObjs {
  ...
  BigInt negative_zero;
  BigInt empty;
  BigInt two_pow_64_minus_2;
  TestObjs();
};
...
void test_u64_ctor_zero(TestObjs *objs);
void test_empty_initlist_ctor(TestObjs *objs);
void test_copy_neg(TestObjs *objs);
void test_get_bit_vector(TestObjs *objs);
void test_is_neg(TestObjs *objs);
void test_minus(TestObjs *objs);
...
int main(int argc, char **argv) {
  ...
  TEST(test_to_hex_1);
  TEST(test_to_hex_2);
  TEST(test_u64_ctor_zero);
  TEST(test_empty_initlist_ctor);
  TEST(test_copy_neg);
  TEST(test_is_neg);
...
TestObjs::TestObjs()
  ...
  , negative_zero(0UL, true)
  , empty({})
  , two_pow_64_minus_2(0xFFFFFFFFFFFFFFFEUL)
{
}
...
void test_u64_ctor_zero(TestObjs *objs) {
  ...
  check_contents(objs->negative_zero, { 0UL });
  ASSERT(objs->negative_zero.is_negative());
}
...
void test_empty_initlist_ctor(TestObjs *objs) {
  check_contents(objs->empty, { });
  ASSERT(!objs->empty.is_negative());
}
...
void test_copy_neg(TestObjs *objs) {
  BigInt copy(objs->negative_nine);

  check_contents(copy, { 9UL  });
  ASSERT(copy.is_negative());
}

void test_get_bit_vector(TestObjs *objs) {
  ASSERT(objs->zero.get_bit_vector() == objs->negative_zero.get_bit_vector());
}
...
void test_mul_3(TestObjs *objs) {
    ...
    BigInt prod1 = objs->zero * objs->zero;
    check_contents(prod1, {0UL});
    ASSERT(!prod1.is_negative());
  
  
    BigInt prod2 = objs->zero * objs->two;
    check_contents(prod2, {0UL});
    ASSERT(!prod2.is_negative());
  
  
    BigInt prod3 = objs->two * objs->zero;
    check_contents(prod3, {0UL});
    ASSERT(!prod3.is_negative());
  
  
    BigInt prod4 = objs->zero * objs->negative_three;
    check_contents(prod4, {0UL});
    ASSERT(!prod4.is_negative());
  
}

void test_mul_4(TestObjs *objs) {

    BigInt prod1 = objs->one * objs->two;
    check_contents(prod1, {2UL});
    ASSERT(!prod1.is_negative());


    BigInt prod2 = objs->two * objs->one;
    check_contents(prod2, {2UL});
    ASSERT(!prod2.is_negative());


    BigInt prod3 = objs->two * objs->two;
    check_contents(prod3, {4UL});
    ASSERT(!prod3.is_negative());


    BigInt prod4 = objs->one * objs->one;
    check_contents(prod4, {1UL});
    ASSERT(!prod4.is_negative());
...
void test_mul_5(TestObjs *objs) {
    ...
    BigInt prod1 = objs->three * objs->three;
    check_contents(prod1, {9UL});
    ASSERT(!prod1.is_negative());


    BigInt prod2 = objs->three * objs->negative_three;
    check_contents(prod2, {9UL});
    ASSERT(prod2.is_negative());


    BigInt prod3 = objs->negative_three * objs->three;
    check_contents(prod3, {9UL});
    ASSERT(prod3.is_negative());


    BigInt prod4 = objs->negative_three * objs->negative_three;
    check_contents(prod4, {9UL});
    ASSERT(!prod4.is_negative());
...
void test_div_3(TestObjs *objs) {
    ...
    try {
      BigInt prod = objs->zero / objs->zero;
      ASSERT(1 == 2);
    } catch (...) {
    }
  
 
    BigInt prod = objs->zero / objs->two;
    check_contents(prod, {0UL});
    ASSERT(!prod.is_negative());
  
  
    try	{
      BigInt prod = objs->two / objs->zero;
      ASSERT(1 == 2);
    } catch (...) {
    }
...
void test_div_4(TestObjs *objs) {
    ...
    BigInt prod5 = objs->two / objs->nine;
    check_contents(prod5, {0UL});
    ASSERT(!prod5.is_negative());
}

void test_div_5(TestObjs *objs) {

    BigInt prod1 = objs->three / objs->three;
    check_contents(prod1, {1UL});
    ASSERT(!prod1.is_negative());


    BigInt prod2 = objs->three / objs->negative_three;
    check_contents(prod2, {1UL});
    ASSERT(prod2.is_negative());
...
void test_lshift_3(TestObjs *objs) {
  ...
  // test shifting a big number which results in higher magnitude shift

  BigInt result = objs->two_pow_64 << 64;
...
void test_to_dec_3(TestObjs *objs) {
  ...
  //positive number
  BigInt val1({0x1234UL});
  std::string result1 = val1.to_dec();
  ASSERT("4660" == result1);

  // zero
  BigInt val2({0x0UL});
  std::string result2 = val2.to_dec();
  ASSERT("0" == result2);

  // negative number
  BigInt val3({0x1234UL}, true);
  std::string result3 = val3.to_dec();
  ASSERT("-4660" == result3);

  // number represented in two 64-bit parts
  BigInt val4({0x1UL, 0x1UL});
  std::string result4 = val4.to_dec();
  ASSERT("18446744073709551617" == result4);

  // negative number represented in two 64-bit parts
  BigInt val5({0x1UL, 0x1UL}, true);
  std::string result5 = val5.to_dec();
  ASSERT("-18446744073709551617" == result5);
}

void test_compare_3(TestObjs *objs) {
  //compare mixed values
  BigInt val1({0x1, 0xFFFFFFUL});
  BigInt val2({0x1, 0xFFFFFFUL});
  BigInt val3({0x2, 0xFFFFFFUL});
  
  ASSERT(val1.compare(val2) == 0);
  ASSERT(val1.compare(val3) < 0);
  ASSERT(val3.compare(val1) > 0);
}
...
void test_is_bit_set_3(TestObjs *objs) {
  ...
  ASSERT(!objs->two_pow_64.is_bit_set(0));
  ASSERT(!objs->two_pow_64.is_bit_set(63));
  ASSERT(objs->two_pow_64.is_bit_set(64));
  ASSERT(!objs->two_pow_64.is_bit_set(65));
  
  //big numbers
  BigInt big_val({0x0UL, 0x0UL, 0x1UL});
  ASSERT(!big_val.is_bit_set(64));
  ASSERT(!big_val.is_bit_set(127));
  ASSERT(big_val.is_bit_set(128));
  ASSERT(!big_val.is_bit_set(129));
}

void test_to_hex_3(TestObjs *) {

  //large numbers
  BigInt val({0xabcdefabcdefabcdUL, 0x1234567890abcdefUL});
  std::string result = val.to_hex();
  ASSERT("1234567890abcdefabcdefabcdefabcd" == result);

  //negative numbers
  BigInt val1({0xabcdefabcdefabcdUL, 0x1234567890abcdefUL}, true);
  std::string result1 = val1.to_hex();
  ASSERT("-1234567890abcdefabcdefabcdefabcd" == result1);
